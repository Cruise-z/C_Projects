...
mpz_gcd_re GF_2_exEuclid(char *binary_s1, char *binary_s2, char *modstr)
{
    ...
    if(binary_s2[0] == '0'){
        result.gcd_str = binary_s1;
        result.x_str = malloc(sizeof(char) * LEN);
        strcpy(result.x_str, "1");
        result.y_str = binary_s2;
        return result;
    }else{
        temp = GF_2_exEuclid(binary_s2, GF_2_poly_division(binary_s1, binary_s2).remain, modstr);
        result.gcd_str = temp.gcd_str;
        result.x_str = temp.y_str;
        result.y_str = GF_2_xor(temp.x_str, GF_2_mul(temp.y_str, GF_2_poly_division(binary_s1, binary_s2).ans, modstr));
        return result;
    }
}
...
mpz_gcd_re GF_2_hex_exEuclid(char *hex_s1, char *hex_s2, char *hex_modstr)
{
    ...
    mpz_init_set_str(invu_gcd, result.gcd_str, 2);
    result.gcd_str = mpz_get_str(result.gcd_str, 16, invu_gcd);
    mpz_init_set_str(invu_s1, result.x_str, 2);
    result.x_str = mpz_get_str(result.x_str, 16, invu_s1);
    mpz_init_set_str(invu_s2, result.y_str, 2);
    result.y_str = mpz_get_str(result.y_str, 16, invu_s2);
    return result;
}
...
mpz_gcd_re gcd_ext(mpz_t A1, mpz_t A2){
    ...
    mpz_gcd_re re;
    mpz_init_set(re.x, inv_ans1);
    mpz_init_set(re.y, inv_ans2);
    mpz_init_set(re.gcd, ans);
    return re;
}
...
mpz_gcd_re str_gcdext(char *s1, char *s2)
{
    ...
    mpz_gcd_re re;
    re.x_str = inv_ans1_str;
    re.y_str = inv_ans2_str;
    re.gcd_str = ans_str;
    return re;
}
...
void CRT_RSA(mpz_t *c,mpz_t *p,mpz_t *q,mpz_t *d,mpz_t *ans)
{
    ...
    Fp(&M1,C1,R1,P);                Fp(&M2,C2,R2,Q);        //M1=C1^R1%P;M2=C2^R2%Q
...
void CRT(mpz_t *r, mpz_t *p, mpz_t src_r, mpz_t src_p){
    mpz_t r1, p1, r2, p2, inv_P1, inv_P2, temp;
    mpz_init(temp);
    mpz_init_set(r1, *r);       mpz_init_set(p1, *p);
    mpz_init_set(r2, src_r);   mpz_init_set(p2, src_p);
    mpz_mul(*p, p1, p2);
    mpz_init(inv_P1);           mpz_init(inv_P2);
    mpz_invert(inv_P1, p2, p1);
    mpz_invert(inv_P2, p1, p2);
    mpz_mul(temp, r1, p2);
    mpz_mod(temp, temp, *p);
    mpz_mul(temp, temp, inv_P1);
    mpz_mod(temp, temp, *p);
    mpz_mul(*r, r2, p1);
    mpz_mod(*r, *r, *p);
    mpz_mul(*r, *r, inv_P2);
